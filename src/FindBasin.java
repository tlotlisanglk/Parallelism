...
class FindBasin extends  RecursiveTask<ArrayList<String>> {
    ...
    protected ArrayList<String> compute() {
        if((COL_HIGH - COL_LOW <= SEQUENTIAL_THRESHOLD) && (ROW_HIGH - ROW_LOW <= SEQUENTIAL_THRESHOLD)){
            ...
            FindBasin left_one = new FindBasin(DATA_ARRAY,ROW_LOW, (ROW_LOW+ROW_HIGH)/2,COL_LOW,(COL_LOW+COL_HIGH)/2);
            FindBasin right_one = new FindBasin(DATA_ARRAY,ROW_LOW, (ROW_LOW+ROW_HIGH)/2,(COL_LOW+COL_HIGH)/2,COL_HIGH);
            FindBasin left_two = new FindBasin(DATA_ARRAY, (ROW_LOW+ROW_HIGH)/2,ROW_HIGH,COL_LOW,(COL_LOW+COL_HIGH)/2);
            FindBasin right_two = new FindBasin(DATA_ARRAY, (ROW_LOW+ROW_HIGH)/2,ROW_HIGH,(COL_LOW+COL_HIGH)/2,COL_HIGH);
